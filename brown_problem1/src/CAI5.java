...
public class CAI5
{
	...
	public static void quiz()									// "Create a method called "quiz" that contains the program logic"
	{
		...
		while (cont==1)											// "The program shall terminate when the student declines to solve another problem set"
		{
			...
			while (i<10)										// "The program shall ask the student to solve 10 different multiplication problems"
			{
				...
				{
					float correctAnswer = askQuestion(a,b,type);							// "The program shall ask the student to solve a multiplication problem";
					float studentAnswer;
	...
	public static int readProblemType()							// "Create a method called "readProblemType" that reads the difficulty level from the student"
	{															// Assuming this was meant to say it selects which type of arithmetic the student wants
		System.out.println("Please select which type of problems you wish to try:");
		...
		System.out.println("5, any type randomly mixed");
		System.out.print("Your selection: ");
		int sel;
		Scanner sc = new Scanner(System.in);
		sel = sc.nextInt();
	...
	public static float askQuestion(int a, int b, int type)			// "Create a method called "askQuestion" that prints the problem to the screen"
	{
		System.out.print("What is " + a);
		switch (type)
		{
			case 1: System.out.print(" plus " + b + "? "); return a+b;
			case 2: System.out.print(" times " + b + "? ");return a*b;
			case 3: System.out.print(" minus " + b + "? ");return a-b;
			case 4: 
			{
				if(b!=0) 											// Make sure it doesnt try to divide by zero
				{
					System.out.print(" divided by " + b + "? (Round DOWN to the nearest whole integer) "); 
					return Math.round(a/b);
				}
				  
				else 												// it tried to divide by zero 
				{
					b++;											// b++ to prevent division by zero
					System.out.print(" divided by " + b + "? (Round DOWN to the nearest whole integer) "); 
					return Math.round(a/b); 					
				}
				
			}
			case 5: 												// nested switch statement to randomly generate the questions
			{
				SecureRandom secureRandom = new SecureRandom();
				int rand = secureRandom.nextInt(4);
				switch (rand)
				{
					case 0: System.out.print(" plus " + b + "? "); return a+b;
					case 1: System.out.print(" times " + b + "? ");return a*b;
					case 2: System.out.print(" minus " + b + "? ");return a-b;
					case 3: 
					{
						if(b!=0) 									// Make sure it doesnt try to divide by zero
						{
						...
						{
							b++; 									// b++ to prevent division by zero
							System.out.print(" divided by " + b+1 + "? (Round DOWN to the nearest whole integer) "); 
							return Math.round(a/b); 			
						}
	...
	public static boolean isAsnwerCorrect(float correctAnswer, float studentAnswer) // "Create a method called "isAnswerCorrect" that checks to see if the student's answer matches the correct answer to the problem"
	{
